// bhg-scanner/scanner.go modified from Black Hat Go > CH2 > tcp-scanner-final > main.go
// Code : https://github.com/blackhat-go/bhg/blob/c27347f6f9019c8911547d6fc912aa1171e6c362/ch-2/tcp-scanner-final/main.go
// License: {$RepoRoot}/materials/BHG-LICENSE
// Useage: run `go build` followed by `./main`
// {TODO 1: FILL IN}

package scanner

import (
	"fmt"
	"net"
	"sort"
	"time"
)



func worker(ports, results chan int) { // cannot update slice values in worker
	for p := range ports {
		address := fmt.Sprintf("scanme.nmap.org:%d", p)  
		conn, err := net.DialTimeout("tcp", address, 20 * time.Second ) // DRMIKE: using DialTimeout over Dial (one extra parameter!); consider < 1 Second
		// conn, err := net.Dial("tcp", address) // TODO 2 : REPLACE THIS WITH DialTimeout (before testing!)
		if err != nil { // got some error (e.g. closed/filtered)
			results <- (-1 * p) // if error is nil, put neg port number in chanel
			continue
		}
		conn.Close()
		results <- p // a chanel of 0s and open port numbers
	}
}

// for Part 5 - consider
// easy: taking in a variable for the ports to scan (int? slice? ); a target address (string?)?
// med: easy + return  complex data structure(s?) (maps or slices) containing the ports.
// hard: restructuring code - consider modification to class/object 
// No matter what you do, modify scanner_test.go to align; note the single test currently fails
func PortScanner(scanPorts int) (int, int) {  

	//TODO 3 : ADD closed ports; currently code only tracks open ports
	var openports []int  // notice the capitalization here. access limited!
	var closedports []int // for closed ports

	ports := make(chan int, 100)   // TODO 4: TUNE THIS FOR CODEANYWHERE / LOCAL MACHINE. can use 99/any number that allows it run
	results := make(chan int)

	for i := 0; i < cap(ports); i++ {
		go worker(ports, results)
	}

	go func() {
		for i := 1; i <= scanPorts; i++ {
			ports <- i
		}
	}()

	for i := 0; i < scanPorts; i++ { // happens asynchronously
		port := <-results // will likely get ports in random order/order at which they finished 
		if port > 0 {
			openports = append(openports, port) // if > 0, add to open ports
		} else if port < 0 {
			closedports = append(closedports, (-1 * port)) // if < 0 add to closedports
		}
	}

	close(ports)
	close(results)
	sort.Ints(openports)
	sort.Ints(closedports)

	//TODO 5 : Enhance the output for easier consumption, include closed ports

	for _, cport := range closedports {
		fmt.Printf("%d, closed\n", cport)
	}

	for _, port := range openports {
		fmt.Printf("%d, open\n", port)
	}

	// fmt.Println("Total: ", len(openports)+len(closedports))
	

	return len(openports), len(closedports) // TODO 6 : Return total number of ports scanned (number open, number closed); 
	//you'll have to modify the function parameter list in the defintion and the values in the scanner_test
}
